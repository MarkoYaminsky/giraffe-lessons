...
def string_multiplication(quantity: int) -> str:
    ...
    your_string = input('Введіть стрічку')
    return your_string * quantity
...
def all_operations_in_one(number1: int, number2: int) -> None:
    ...
    """
    print(number1 * number2, number1 / number2, number1 + number2, number1 - number2, sep=',', end=',')
    print(number1 ** number2, number2 ** number1, number2 // number1, number1 // number1, sep=',', end=',')
    print(number1 % number2, number2 % number1, sep=',')
...
def only_positive(number1: int, number2: int, number3: int) -> int:
    ...
    sum_number = 0
    if number1 > 0:
        sum_number = sum_number + number1
    if number2 > 0:
        sum_number = sum_number + number2
    if number3 > 0:
        sum_number = sum_number + number3
    return sum_number
...
def beyond_two(lower_limit: int, upper_limit: int, number: int) -> bool:
    ...
    your_number = int(input("Введіть ваше число"))
    return lower_limit < your_number < upper_limit
...
def pretty_number(questionable_number: int) -> bool:
    ...
    modulo_division_by7 = questionable_number % 7
    modulo_division_by17 = questionable_number % 17

    if modulo_division_by7 == 0 and \
            modulo_division_by17 == 0 and \
            questionable_number // 1000 != 0 and \
            questionable_number // 10000 == 0:
        return True
    return False
